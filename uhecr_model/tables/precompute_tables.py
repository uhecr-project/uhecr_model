...
parser = argparse.ArgumentParser(
    description="Precompute exposure integral and energy grid."
)
parser.add_argument(
    "--source",
    dest="source_type",
...
if __name__ == "__main__":
    ...
    table_file = os.path.join(
        path_to_this_file,
        "tables_{0}_{1}.h5".format(args.source_type, args.detector_type),
    )

    # get things related to detector
    detector_properties, Eth = get_detectorimports(args.detector_type)
    ...
    analysis = Analysis(
        data, model, analysis_type="joint", filename=table_file, summary=summary
    )
